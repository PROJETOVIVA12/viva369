...
import {
  Trophy, Gift, Plus, Zap, 
  Users, Crown, Star, Sparkles, Award,
  CheckCircle, Loader2, Bell, Rocket,
  Flame, Medal, Gem
} from 'lucide-react';
...
interface SistemaPremiosProps {
  desafioId: string;
  tipo: 'individual' | 'grupo';
  onPremioAdicionado?: () => void;
}

export default function SistemaPremios({ desafioId, tipo, onPremioAdicionado }: SistemaPremiosProps) {
  const { user } = useAuth();
  ...
  const criarPremio = async () => {
    ...
    try {
      ...
      setMensagem({ tipo: 'sucesso', texto: '✅ Prêmio adicionado com sucesso!' });
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          premio_id: premio.id,
          tipo: 'turbinado',
          mensagem: `🎉 O prêmio "${premio.nome}" foi turbinado para "${turbinado.nome}"!`
        });

      setMensagem({ 
        tipo: 'sucesso', 
        texto: `🚀 Prêmio turbinado para ${turbinado.nome}!` 
      });
      setTimeout(() => setMensagem(null), 3000);
      carregarPremios();
    } catch (error) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-white font-medium">Prêmios do Desafio</h3>
          {tipo === 'grupo' && (
        ...
        <Button
          ...
          onClick={() => { setFormData({ nome: '', descricao: '', valor_estimado: '' }); setShowDialog(true); }}
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-8 bg-slate-700/20 rounded-lg border border-slate-600">
          ...
          <p className="text-slate-400">Nenhum prêmio adicionado ainda</p>
          <p className="text-xs text-slate-500">
            {tipo === 'individual' ? 'Adicione um prêmio para motivar seu desafio!' : 
             'O líder do grupo deve adicionar os prêmios!'}
          </p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {isTurbinado && premio.turbinado_por && (
                  ...
                  {isAdmin && !isTurbinado && (
                    ...
                    >
                      ...
                      <span className="text-xs">Turbinar</span>
                    </Button>
                  )}
                </div>
              </div>
        ...
        <DialogContent className="bg-slate-800 border-slate-700 max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              {tipo === 'individual' 
                ? 'Escolha um prêmio para motivar seu desafio!' 
                : 'Como líder do grupo, escolha o prêmio para o desafio em grupo!'}
            </DialogDescription>
          </DialogHeader>
